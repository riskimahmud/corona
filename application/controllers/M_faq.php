...
class M_faq extends CI_Controller {
	...
	public function tambah_aksi(){
		...
		if($tambah){
			$notifikasi		=	array(
				"status"	=>	1, "pesan"	=>	"FAQ Berhasil Ditambah"
			);
		}else{
			$notifikasi		=	array(
				"status"	=>	0, "pesan"	=>	"FAQ Gagal Ditambah"
			);
		}
	...
	public function perbarui_aksi(){
		...
		if($perbarui){
			$notifikasi		=	array(
				"status"	=>	1, "pesan"	=>	"FAQ Berhasil Diperbarui"
			);
		}else{
			$notifikasi		=	array(
				"status"	=>	0, "pesan"	=>	"FAQ Gagal Diperbarui"
			);
		}
		$this->session->set_flashdata("notifikasi",$notifikasi);
		redirect($this->base);
